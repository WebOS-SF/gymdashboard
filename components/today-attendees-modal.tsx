...
import { X, CheckCircle2, XCircle, CircleDot, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function TodayAttendeesModal({ clients, isOpen, onClose }: TodayAttendeesModalProps) {
  ...
  const [attendances, setAttendances] = useState<{ clientDni: number; status: AttendanceStatus }[]>([])
  ...
  useEffect(() => {
    ...
    const fetchAttendances = async () => {
      ...
      try {
        ...
        const peruTime = new Date(selectedDate.getTime() - (5 * 60 * 60 * 1000))
        const dateStr = `${peruTime.getUTCFullYear()}-${String(peruTime.getUTCMonth() + 1).padStart(2, '0')}-${String(peruTime.getUTCDate()).padStart(2, '0')}`
  ...
  const attendanceMap = new Map(attendances.map(a => [a.clientDni, a.status]))
  const attendees = clients.filter((client) => {
    const status = attendanceMap.get(Number(client.dni))
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-3xl max-h-[80vh] overflow-y-auto border-border bg-card">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-foreground">Asistencias</CardTitle>
            <CardDescription className="text-muted-foreground">
              {attendees.length} cliente{attendees.length !== 1 ? 's' : ''} con registro
            </CardDescription>
          </div>
        ...
        <CardContent className="space-y-4">
          {/* Selector de fecha */}
          <div className="flex items-center justify-between bg-secondary/30 rounded-xl px-3 py-1.5">
            <button
              onClick={goToPrevDay}
              className="p-0.5 rounded-lg hover:bg-secondary/50 transition-colors"
            ...
            <button
              onClick={goToNextDay}
              disabled={isToday}
              className="p-0.5 rounded-lg hover:bg-secondary/50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          {attendees.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              {isLoading ? 'Cargando...' : 'No hay registros de asistencia para esta fecha'}
            </div>
          ) : (
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            {getAttendanceIcon(status || 'NONE')}
                            {getStatusBadge(status || 'NONE')}
                          </div>
                        </TableCell>
