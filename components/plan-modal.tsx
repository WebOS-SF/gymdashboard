...
export function PlanModal({ mode, clients, client, plan, canViewMoney, isOpen, isSaving, onClose, onSave, onDelete }: PlanModalProps) {
  ...
  const hasValidDaySelection = useMemo(() => {
    ...
    if (formData.planTier === 'diario_escolar_universitario') return formData.attendanceDays.length === 6
  ...
  const validationMessage = useMemo(() => {
    ...
    if (formData.planTier === 'diario_escolar_universitario' && formData.attendanceDays.length !== 6) {
      return 'Mensualidad Diaria Escolares/Universitarios: Debes marcar los 6 días (lun-sab)'
    }
  ...
  const handlePlanTierChange = (planTier: PlanTier) => {
    setFormData((current) => {
      ...
      } else if (planTier === 'diario' || planTier === 'diario_trotadora' || planTier === 'diario_escolar_universitario') {
        updates.durationValue = 1
        updates.durationUnit = 'month'
        updates.attendancePreset = 'daily'
        updates.attendanceDays = defaultAttendanceDays
                ...
                <Field>
                  ...
                  <div className="grid grid-cols-[120px_1fr] gap-2">
                    ...
                    <Select
                      ...
                      disabled={formData.planTier !== 'basic' && formData.planTier !== 'diario_escolar_universitario'}
                    >
                      <SelectTrigger className="bg-input border-border text-foreground">
                        <SelectValue />
                      </SelectTrigger>
